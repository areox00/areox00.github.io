...
**Warning!** While using the callback you need to be careful because audio works on another thread therefore passing data via `spec.userdata` might led to data races if you don't use any form of locking (such as std::mutex), in **this case** it's not that bad but keep it in mind.
