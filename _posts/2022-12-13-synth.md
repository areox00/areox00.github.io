...
With callback function you might create data race because audio works on another thread, tbh they are quite harmless, if you wish you can omit them.
...
From now we can start making synthesizer, lets start by generating waveform. Waveform is just graph which represent sound.
...
Well if you won't check for it in `SDL_KEYUP` you could easily interrupt playing by releasing old key.
...
Other important thing is that I didn't implemented any anti-aliasing filter so aliasing will occur, I will take care of that in next parts.
